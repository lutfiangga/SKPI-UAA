						...
						foreach ($read as $row) { ?>
							<tr class="border-t">
								...
								<td class="p-2 flex flex-row items-center mt-2 gap-2">
									<a href="<?= site_url(ucwords($role) . '/Skor_Minimum_Spm/edit/' . $row['id_skor']); ?>" class="bg-green-600 rounded-full p-2 text-[#fafafa] hover:px-4 flex items-center gap-2 group">
										<i data-feather="edit" class="w-4 h-auto"></i>
										<p class="hidden group-hover:block text-white transition-opacity duration-300">Edit</p>
									</a>

									<button class="bg-red-600 rounded-full p-2 text-[#fafafa] hover:px-4 flex items-center gap-2 group" onclick="openDeleteModal('<?= $row['id_skor']; ?>')">
