<section class="relative flex flex-col justify-between">
	<div class="flex flex-row justify-between items-center">
		<div class="flex flex-row justify-start items-center md:gap-4 gap-2">
			...
			<div class="bg-blue-600 md:p-3 p-2 text-[#fafafa] rounded-lg">
				<i data-feather="crosshair" class="w-4 h-4 md:h-[24px] md:w-[24px]"></i>
			</div>
		</div>
	...
	<section class="relative bg-[#fafafa] rounded-2xl lg:p-8 p-4 my-4">
		<!-- add user -->
		<a href="<?= site_url(ucwords($role) . '/Skor_Minimum_Spm/create'); ?>" class="btn bg-blue-600 border-none text-[#fafafa] hover:bg-[#fafafa]/30 hover:text-blue-600 hover:border-2 hover:border-blue-600 hover:shadow-md mb-4 w-full md:w-auto flex flex-row items-center max-w-xs">
				...
				<tbody>
					<?php if (!empty($read)) {
						...
						foreach ($read as $row) { ?>
							<tr class="border-t">
								<td class="p-2"><?= $no; ?></td>
								<td class="p-2 whitespace-normal"><?= $row['tingkat_jenjang'] . ' | ' . $row['jenjang'] ?></td>
								<td class="p-2 whitespace-normal"><?= $row['parameter'] . ' ' . $row['angkatan'] ?></td>
								<td>
								...
								<td class="p-2 flex flex-row items-center mt-2 gap-2">
									<a href="<?= site_url(ucwords($role) . '/Skor_Minimum_Spm/edit/' . $row['id_skor']); ?>" class="rounded-full p-2 bg-orange-100 text-orange-600 hover:scale-125 hover:bg-orange-200 flex items-center gap-2">
										<i data-feather="edit" class="w-4 h-auto"></i>
									</a>

									<button class="rounded-full p-2 bg-red-100 text-red-600 hover:scale-125 hover:bg-red-200 flex items-center gap-2" onclick="openDeleteModal('<?= $row['id_skor']; ?>')">
...
<script>
	...
	const openDeleteModal = (id) => {
		document.getElementById('hapus_id_skor').value = id;
