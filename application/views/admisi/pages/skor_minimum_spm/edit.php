<section class="relative flex flex-col justify-between">
	...
	<section class="relative bg-[#fafafa] rounded-2xl lg:p-8 p-4 my-4">
		<form method="post" action="<?= site_url(ucwords($role) . '/Skor_Minimum_Spm/update/' . $edit['id_skor']) ?>"
			...
			<div class="grid md:grid-cols-2 gap-2 md:gap-4">
				<div class="">
					...
					<select id="jenjang" name="id_jenjang" class="block bg-off-white w-full mt-1 p-2 border border-gray-300 rounded-md" data-search="true">
						<option value="" selected disabled>--Pilih Jenjang--</option>
						<?php foreach ($jenjang as $r) { ?>
							<option value="<?= $r['id_jenjang'] ?>" <?= ($r['id_jenjang'] == $edit['id_jenjang']) ? 'selected' : '' ?>><?= $r['tingkat_jenjang'] . ' | ' . $r['jenjang']; ?></option>
						<?php } ?>
					</select>
				</div>
				<div class="">
					<label for="parameter" class="block text-sm font-medium text-gray-700 mb-2">Parameter:</label>
					<select id="parameter" name="parameter" class="block bg-off-white w-full mt-1 p-2 border border-gray-300 rounded-md" data-search="true">
						<option value="" selected disabled>--Pilih parameter--</option>
						<option value="=" <?= ($edit['parameter'] == '=') ? 'selected' : '' ?>>(=) Sama Dengan</option>
						<option value="!=" <?= ($edit['parameter'] == '!=') ? 'selected' : '' ?>>(!=) Tidak Sama Dengan</option>
						<option value=">" <?= ($edit['parameter'] == '>') ? 'selected' : '' ?>>(>) Lebih Besar</option>
						<option value="<" <?= ($edit['parameter'] == '<') ? 'selected' : '' ?>>(<) Lebih Kecil</option>
						<option value=">=" <?= ($edit['parameter'] == '>=') ? 'selected' : '' ?>>(>=) Lebih Besar atau Sama Dengan</option>
						<option value="<=" <?= ($edit['parameter'] == '<=') ? 'selected' : '' ?>>(<=) Lebih Kecil atau Sama Dengan</option>
					</select>
				</div>
				<div class="mb-4">
					<label for="angkatan" class="block text-sm font-medium text-gray-700 mb-2">Angkatan:</label>
					<input type="text" id="angkatan" name="angkatan" value="<?= $edit['angkatan']; ?>"
				...
				<div class="mb-4">
					...
					<input type="number" inputmode="numeric" id="skor" name="skor" value="<?= $edit['skor']; ?>"
						class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-500 focus:border-blue-500 p-2"
