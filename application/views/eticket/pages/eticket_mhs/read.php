<section class="relative flex flex-col justify-between">
							...
							<tr class="border-t">
								...
								<td class="px-4 py-2 flex flex-row items-center mt-2 gap-2">
									<button class="bg-green-600 rounded-full p-2 text-[#fafafa] hover:px-4 flex items-center gap-2 group"
										onclick="openEditModal('<?= $row['id_etiket']; ?>', '<?= $row['nim']; ?>', '<?= $row['jenis_pelanggaran']; ?>', '<?= $row['pelanggaran']; ?>', '<?= $row['poin']; ?>', '<?= $row['bukti']; ?>')">
										<i data-feather="edit" class="w-4 h-auto"></i>
										<p class="hidden group-hover:block text-white transition-opacity duration-300">Edit</p>
									</button>

									<button class="bg-red-600 rounded-full p-2 text-[#fafafa] hover:px-4 flex items-center gap-2 group" onclick="openDeleteModal(<?= $row['id_etiket']; ?>)">
	...
	<dialog id="addEtiket" class="modal overflow-hidden">
		<div class="modal-box bg-[#fafafa] w-full sm:w-11/12 lg:max-w-5xl mx-auto p-4">
			...
			<form method="post" action="<?= site_url('Eticket/Eticket_Mahasiswa/save'); ?>" enctype="multipart/form-data" role="form">
				<?= csrf(); ?>
	...
	<dialog id="editEtiket" class="modal overflow-hidden">
		<div class="modal-box bg-[#fafafa] w-11/12 max-w-5xl">
			...
			<h3 class="text-lg font-bold text-blue-600 flex flex-row items-center">
				Edit Eticket Mahasiswa
				<div class="bg-blue-600 md:p-3 p-2 text-[#fafafa] rounded-lg ml-2 md:ml-4">
					<i data-feather="activity" class="w-4 h-4"></i>
				</div>
			</h3>
			<div class="divider border-gray-400"></div>
			<form method="post" action="<?= site_url('Eticket/Eticket_Mahasiswa/update'); ?>" enctype="multipart/form-data" role="form">
				<?= csrf(); ?>
				<input type="hidden" id="edit_id_etiket" name="id_etiket" value="" />
	...
	<dialog id="hapusEtiket" class="modal overflow-hidden">
		<div class="modal-box bg-[#fafafa]">
			...
			<form method="post" action="<?= site_url('Eticket/Eticket_Mahasiswa/delete'); ?>">
				<?= csrf(); ?>
				<input type="hidden" id="hapus_id_etiket" name="id_etiket" />
				<div class="modal-action relative" style="z-index: 1000;">
...
<script>
	const openEditModal = (id, mhs, jenis, pelanggaran, poin, bukti) => {
		document.getElementById('edit_id_etiket').value = id;
		...
		filePreview.textContent = bukti; // Menampilkan nama file yang sudah ada
	...
	const openDeleteModal = (id) => {
		document.getElementById('hapus_id_etiket').value = id;
