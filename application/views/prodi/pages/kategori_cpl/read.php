					...
					foreach ($read as $row) { ?>
						<tr class="border-t">
							...
							<td class="p-2 flex flex-row items-center justify-center mt-2 gap-2">
								<a href="<?php echo site_url('Prodi/Kategori_Cpl/edit/' . $row['id_kategori_cpl']); ?>" class="bg-green-600 rounded-full p-2 text-[#fafafa] hover:px-4 flex items-center gap-2 group">
									<i data-feather="edit" class="w-4 h-auto"></i>
									<p class="hidden group-hover:block text-white transition-opacity duration-300">Edit</p>
								</a>
								<button type="button" onclick="openDeleteModal('<?= $row['id_kategori_cpl']; ?>')" class="bg-red-600 rounded-full p-2 text-[#fafafa] hover:px-4 flex items-center gap-2 group">
