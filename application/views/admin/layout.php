...
<head>
	...
	<!-- DaisyUI -->
	<!-- <link rel="stylesheet" href="<?= base_url(); ?>assets/vendor/css/daisy.css" /> -->
	<!-- Custom CSS -->
...
<body>
	...
	<div class="flex pt-16"> <!-- Wrapper for sidebar and main content -->

		<!-- Sidebar -->
		<aside class="sidebar bg-blue-600 mt-8 rounded-tr-[4rem] flex-shrink-0 fixed top-16 bottom-0 -left-64 lg:left-0 p-4 w-[250px] lg:w-[300px] overflow-y-auto text-center transition-all duration-300 z-40">
			<nav class="text-gray-800 text-xl relative">
				<ul class="mt-8 text-[#fafafa] mr-6">
					...
					<!-- data pengguna -->
					<li class="my-2">
						<button class="p-2.5 font-semibold mt-3 flex items-center rounded-full px-2 whitespace-nowrap lg:px-6 duration-300 hover:bg-blue-600 focus:ring-2 focus:ring-[#dadada] focus:bg-[#fafafa] focus:text-blue-600 text-[#fafafa] w-full" onclick="toggleDropdown(this)" aria-expanded="false">
							<i data-feather="users"></i>
							<div class="flex justify-between w-full items-center">
								<span class="text-[15px] ml-4">Data Pengguna</span>
								<span class="text-sm transition-transform duration-300 dropwdown">
									<i data-feather="chevron-down"></i>
								</span>
							</div>
						</button>
						<!-- Dropdown Menu -->
						<ul class="text-left text-sm rounded-full font-thin mt-2 w-4/5 mx-auto hidden" aria-label="Menu 1 Options">
							<!-- mahasiswa -->
							<li>
								<a href="#" class="block flex flex-row gap-2 items-center cursor-pointer p-2 hover:bg-blue-600 focus:ring-2 focus:ring-[#dadada] focus:bg-[#fafafa] focus:text-blue-600 text-[#fafafa] rounded-full mt-1 font-semibold">
		...
		<!-- Main Content -->
		<main class="flex-grow p-8 lg:ml-[300px] ml-0 transition-all duration-300">
			<section class="">
				...
				<section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 my-4 lg:my-8 md:my-6">
					<!-- card -->
					<div class="flex flex-col col-span-1 text-[#fafafa]">
						<div class="bg-blue-600 rounded-t-2xl flex flex-row p-2 w-3/4" style="clip-path: polygon(0 0, 83% 0, 100% 100%, 0% 100%);">
						...
						<div class="bg-blue-600 rounded-b-2xl rounded-tr-2xl p-4">
							<div class="flex flex-row justify-between mx-2 items-center gap-2">
								<div>
									<div class="bg-[#fafafa] rounded-md text-blue-600 p-4">
							...
							<div class="flex flex-row gap-4 items-center mt-6">
								<i data-feather="clock" class="text-gray-300/80"></i>
								<div class="flex flex-col text-gray-300/80 uppercase tracking-wide items-start">
	...
	<script>
		const toggleDropdown = (element) => {
			...
			const dropdown = element.querySelector('.dropwdown');
		...
		(function(d) {
			...
			d.extend(x.prototype, {
				...
				text: function(a, b) {
					...
					a =
						c.inCollection && a && a.tag ? a.tag : this.c.dom.buttonLiner.tag;
					var e = this.s.dt,
						h = d(c.node),
						f = function(g) {
				...
				_popover: function(a, b, c, e) {
					...
					if (!1 === a) l();
						...
						c.length &&
							(g.closest("div.dt-button-collection").length && (g = c.eq(0)),
								l());
					...
					var c = [],
						...
						h = function(l, m) {
							...
							if (Array.isArray(l) || l instanceof d)
								...
								for (
									l = l.replace(":name", ""), n = 0, r = q.length; n < r; n++
								)
									q[n].name === l && c.push({
										inst: m,
										node: q[n].node
			...
			x.defaults = {
				...
				dom: {
					...
					collection: {
						tag: "div",
						className: "absolute z-10 mt-2 origin-top rounded-md bg-white shadow-lg ring-opacity-5 w-[8vw]",
					},
					button: {
						tag: "button",
						className: "inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-3 text-sm font-semibold text-gray-900 shadow-sm hover:bg-gray-50",
					...
					splitDropdown: {
						tag: "button",
						text: '<svg class="-mr-1 h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd" /></svg>',
						className: "inline-flex w-full justify-center px-4 py-2",
						align: "split-left",
						splitAlignClass: "absolute left-0 top-full mt-1 w-full",
					},
					splitDropdownButton: {
						tag: "button",
		...
		$(document).ready(function() {
			...
			dateFilter.change(function() {
				if ($(this).val() === "custom") {
					...
					customDate.hide();
					filterTableByDate($(this).val());
				}
			});

			startDateInput.add(endDateInput).change(function() {
				if (dateFilter.val() === "custom") {
					filterTableByDate("custom");
				}
			});

			function filterTableByDate(filter) {
				const now = new Date();
	...
	<script>
		$(document).ready(function() {
			$("table.table-data").DataTable({
				...
				dom: "<'flex justify-between items-center mb-4 w-full'<'flex items-center w-2/3 sm:w-3/4'><'flex items-center space-x-2'fB>>" +
					"<'w-full'tr>" +
					"<'flex justify-between items-center mt-4 w-full'<'flex items-center space-x-2'l><'flex items-center mt-2'i><'flex items-center'p>>",
				buttons: [{
					extend: "collection",
					text: "Export",
