<section class="relative flex flex-col justify-between">
	...
	<section class="relative bg-[#fafafa] rounded-2xl lg:p-8 p-4 my-4">
		<!-- add user -->
		<a href="<?= site_url('Admin/Account/Admin/create'); ?>" class="btn max-w-[15rem] bg-blue-600 border-none text-[#fafafa] hover:bg-[#fafafa]/30 hover:text-blue-600 hover:border-2 hover:border-blue-600 hover:shadow-md mb-4 md:w-auto flex flex-row items-center">
					...
					?>
						<tr class="border-t">
							...
							<td class="px-4 py-2 flex flex-row items-center justify-center mt-2 gap-2">
								...
								<a title="Edit" href="<?= site_url('Admin/Account/Admin/edit/' . $row['id_akun']); ?>" class="rounded-full p-2 bg-orange-100 text-orange-600 hover:scale-125 hover:bg-orange-200 flex items-center gap-2">
									<i data-feather="edit" class="w-4 h-auto"></i>
								</a>
								<button title="Delete" type="button" onclick="openDeleteModal('<?= $row['id_akun']; ?>')" class="rounded-full p-2 bg-red-100 text-red-600 hover:scale-125 hover:bg-red-200 flex items-center gap-2">
	...
	<dialog id="hapusAkun" class="modal overflow-hidden">
		<div class="modal-box bg-[#fafafa]">
			...
			<form method="post" action="<?= site_url('Admin/Account/Admin/delete/'); ?>">
				<?= csrf(); ?>
	...
	<dialog id="resetPassword" class="modal overflow-hidden">
		<div class="modal-box bg-[#fafafa]">
			...
			<form method="post" action="<?= site_url(ucwords($role) . '/Account/Admin/resetPassword/'); ?>">
				<?= csrf(); ?>
