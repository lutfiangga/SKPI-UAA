<section class="relative flex flex-col justify-between">
	...
	<section class="relative bg-[#fafafa] rounded-2xl lg:p-8 p-4 my-4">
		<form method="post" action="<?= site_url(ucwords($role) . '/Account/Mahasiswa/update/' . $edit['id_akun']) ?>"
				...
				<div class="">
					...
					<select id="select_mahasiswa" name="id_user" class="block bg-off-white w-full mt-1 p-2 border border-gray-300 rounded-md" data-search="true">
						...
						<?php foreach ($mhs as $r) { ?>
							<option value="<?= $r['nim'] ?>" <?= ($r['nim'] == $edit['id_user']) ? 'selected' : '' ?>><?= $r['nim'] . ' | ' . $r['nama']; ?></option>
						<?php } ?>
					</select>
				</div>
				<div class="">
			...
			<div class="modal-action relative" style="z-index: 1000;">
				<a href="<?= site_url(ucwords($role) . '/Account/Mahasiswa'); ?>"
