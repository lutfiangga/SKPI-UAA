					...
					?>
						...
						<dialog id="addAkun" class="modal overflow-hidden">
							<div class="modal-box bg-[#fafafa] w-full sm:w-11/12 lg:max-w-5xl mx-auto p-4">
								...
								<?php endif; ?>
								<form method="post" action="<?= site_url('Admin/Account/Mahasiswa/save'); ?>" enctype="multipart/form-data" role="form">
									<?= csrf(); ?>
									<div class="mb-4">
										<label for="select_mahasiswa" class="block text-sm font-medium text-gray-700 mb-2">Mahasiswa:</label>
										<select id="select_mahasiswa" name="id_user" class="block bg-off-white w-full mt-1 p-2 border border-gray-300 rounded-md" data-search="true">
						...
						<dialog id="editAkun" class="modal overflow-hidden">
							<div class="modal-box bg-[#fafafa] w-11/12 max-w-5xl">
								...
								<?php endif; ?>
								<form method="post" action="<?= site_url('Admin/Account/Mahasiswa/update/' . $row['id_akun']); ?>" enctype="multipart/form-data" role="form">
									<?= csrf(); ?>
									...
									<div class="mb-4">
										...
										<select id="mahasiswa" name="id_user" class="block bg-off-white w-full mt-1 p-2 border border-gray-300 rounded-md" data-search="true">
											<option value="" selected disabled>--Pilih Mahasiswa--</option>
											<?php foreach ($mhs as $r) { ?>
						...
						<dialog id="hapusAkun" class="modal overflow-hidden">
							<div class="modal-box bg-[#fafafa]">
								...
								<form method="post" action="<?= site_url('Admin/Account/Mahasiswa/delete/'); ?>">
									<?= csrf(); ?>
									<input id="id" type="text" name="id_akun">
									<div class="modal-action relative" style="z-index: 1000;">
...
<script>
	const openEditModal = (id, mhs, username, role) => {
		document.getElementById('id_akun').value = id;
		document.getElementById('user').value = username;
		document.getElementById('role').value = role;

		const dataMhs = document.getElementById('mahasiswa');
		dataMhs.value = mhs;
		dataMhs.dispatchEvent(new Event('change'));
