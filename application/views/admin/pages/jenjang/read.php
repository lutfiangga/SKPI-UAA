<section class="relative flex flex-col justify-between">
	...
	<section class="relative bg-[#fafafa] rounded-2xl lg:p-8 p-4 my-4">
		<!-- add jenjang -->
		<a href="<?= site_url('Admin/Jenjang/create'); ?>" class="btn max-w-[15rem] bg-blue-600 border-none text-[#fafafa] hover:bg-[#fafafa]/30 hover:text-blue-600 hover:border-2 hover:border-blue-600 hover:shadow-md mb-4 md:w-auto flex flex-row items-center">
		...
		<div class="overflow-x-auto">
			<table id="" class="min-w-full table-auto table-data">
				<thead class="bg-gray-100">
					<tr>
						<th class="p-2">No</th>
						<th class="p-2 whitespace-normal">Tingkat Jenjang</th>
						<th class="p-2 whitespace-normal">Jenjang</th>
						<th class="p-2 whitespace-normal">Jenjang Lanjutan</th>
						<th class="p-2 whitespace-normal">Jenjang Kuliafikasi KKNI</th>
				...
				<tbody>
					...
					foreach ($read as $row) { ?>
						<tr class="border-t">
							<td class="p-2"><?= $no; ?></td>
							<td class="p-2 whitespace-nowrap"><?= $row['tingkat_jenjang']; ?></td>
							<td class="p-2 whitespace-nowrap"><?= $row['jenjang']; ?></td>
							<td class="p-2 whitespace-nowrap"><?= $row['jenjang_lanjutan']; ?></td>
							<td class="p-2 whitespace-nowrap">Jenjang <?= $row['jenjang_kualifikasi']; ?></td>
							<td class="p-2 flex flex-row items-center justify-center mt-2 gap-2">
								<a href="<?php echo site_url('Admin/Jenjang/edit/' . $row['id_jenjang']); ?>" class="rounded-full p-2 bg-orange-100 text-orange-600 hover:scale-125 hover:bg-orange-200 flex items-center gap-2">
									<i data-feather="edit" class="w-4 h-auto"></i>
								</a>
								<button type="button" onclick="openDeleteModal('<?= $row['id_jenjang']; ?>')" class="rounded-full p-2 bg-red-100 text-red-600 hover:scale-125 hover:bg-red-200 flex items-center gap-2">
						...
						<!-- Modal Hapus Jenjang -->
						<dialog id="hapusJenjang" class="modal overflow-hidden">
							<div class="modal-box bg-[#fafafa]">
								...
								<form method="post" action="<?= site_url('Admin/Jenjang/delete/'); ?>">
									<?= csrf(); ?>
									<input id="id" type="text" name="id_jenjang" hidden>
...
<script>
	const openDeleteModal = (id) => {
		...
		document.getElementById('hapusJenjang').showModal();
	}
</script>
