<section class="relative flex flex-col justify-between">
	...
	<section class="relative bg-[#fafafa] rounded-2xl lg:p-8 p-4 my-4">
		<form method="post" action="<?= site_url(ucwords($role) . '/Subkategori_Spm/save') ?>"
			enctype="multipart/form-data" role="form">
			<?= csrf(); ?>
			<div>
				<div class="mb-2">
					<label for="id_kategori_spm" class="block text-sm font-medium text-gray-700 mb-2">Kategori:</label>
					<select id="id_kategori_spm" required name="id_kategori_spm" class="block bg-off-white w-full mt-1 p-2 border border-gray-300 rounded-md capitalize" data-search="true">
						<option value="" selected disabled>--Pilih kategori--</option>
						<?php foreach ($kategori as $r) { ?>
							<option value="<?= $r['id_kategori_spm'] ?>"><?= $r['kategori']; ?></option>
						<?php } ?>
					</select>
				</div>
				<div class="mb-4">
					<label for="name" class="block text-sm font-medium text-gray-700 mb-2">Nama Subkategori:</label>
					<input type="text" id="name" name="subkategori" required
