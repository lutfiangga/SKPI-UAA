						...
						foreach ($read as $row) { ?>
							<tr class="border-t">
								...
								<td class="p-2 flex flex-row items-center mt-2 gap-2">
									<a href="<?= site_url(ucwords($role) . '/Kategori_Spm/edit/' . $row['id_kategori_spm']); ?>" class="rounded-full p-2 bg-orange-100 text-orange-600 hover:scale-125 hover:bg-orange-200 flex items-center gap-2">
										<i data-feather="edit" class="w-4 h-auto"></i>
									</a>

									<button class="rounded-full p-2 bg-red-100 text-red-600 hover:scale-125 hover:bg-red-200 flex items-center gap-2" onclick="openDeleteModal('<?= $row['id_kategori_spm']; ?>')">
