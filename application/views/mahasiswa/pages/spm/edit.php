		...
		<form method="post" action="<?= site_url(ucwords($role) . '/Spm_Mahasiswa/update/') . $edit['id_spm'] ?>" enctype="multipart/form-data" role="form">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
				...
				<div class="flex flex-col">
					...
					<input type="text" required id="nama_kegiatan" name="nama_kegiatan" class="p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" value="<?= $edit['nama_kegiatan']; ?>">
				</div>

				<!-- Penyelenggara -->
				<div class="flex flex-col">
					<label for="penyelenggara" class="mb-2 text-sm font-medium text-gray-700">Penyelenggara:</label>
					<input type="text" required id="penyelenggara" name="penyelenggara" class="p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" value="<?= $edit['penyelenggara']; ?>">
				</div>

				<!-- Tanggal -->
				<div class="flex flex-col relative">
					<label for="tanggal_mulai" class="mb-2 text-sm font-medium text-gray-700">Tanggal Mulai</label>
					<input type="text" id="tanggal_mulai" required name="tanggal_mulai" class="pickdate p-2 w-full rounded-md bg-[#fafafa] focus:ring-inset focus:ring-blue-400 border border-gray-300" placeholder="Tanggal Mulai" value="<?= $edit['tanggal_mulai']; ?>">
				</div>

				<div class="flex flex-col relative">
					<label for="tanggal_selesai" class="mb-2 text-sm font-medium text-gray-700">Tanggal Selesai:</label>
					<input type="text" id="tanggal_selesai" name="tanggal_selesai" class="pickdate p-2 w-full rounded-md bg-[#fafafa] focus:ring-inset focus:ring-blue-400 border border-gray-300" placeholder="tanggal Selesai" value="<?= $edit['tanggal_selesai']; ?>">
				</div>

				<!-- Link Kegiatan -->
				<div class="flex flex-col">
					<label for="url" class="mb-2 text-sm font-medium text-gray-700">Link Kegiatan:</label>
					<input type="url" required id="url" name="link_kegiatan" class="p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" value="<?= $edit['link_kegiatan']; ?>" placeholder="https://example.com" oninput=" inputValidation(this)">
				...
				<div class="flex flex-col">
					...
					<div class="relative file-upload-container">
						<div class="bg-white drop-zone relative cursor-pointer transition-all text-center p-8 border-2 border-blue-400 border-2 border-dashed rounded-lg w-full shadow-lg transtition">
							...
							<input id="sertifikat" required value="<?= $edit['sertifikat']; ?>" name="sertifikat" accept="application/pdf" type="file" class="file-input hidden" />
				...
				<div class="flex flex-col">
					...
					<div class="relative file-upload-container">
						...
						<div class="file-preview w-full">
							...
							<div class="preview-container w-full flex justify-center mx-auto">
								<?php
								$file_path = FCPATH . 'assets/static/spm/img/foto_kegiatan/' . $edit['foto_kegiatan'];
								if (isset($edit['foto_kegiatan']) && !empty($edit['foto_kegiatan']) && file_exists($file_path)): ?>
									<iframe src="<?= base_url('assets/static/spm/img/foto_kegiatan/' . $edit['foto_kegiatan']); ?>" width="100%" height="400px"></iframe>
								<?php else: ?>
									<p>File tidak ditemukan.</p>
								<?php endif; ?>
							</div>
