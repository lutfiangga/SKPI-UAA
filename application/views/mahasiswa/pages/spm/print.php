			...
			<td class="content-print">
				<main class="container mx-auto p-3 md:p-6 text-black print-content">
					...
					<div class="text-center">
						<h2 class="text-2xl font-semibold uppercase">Formulir Pelaporan</h2>
						<p class="mt-2">Skor Prestasi Mahasiswa (SPM)</p>
					</div>
					<!-- end title -->
					...
					<div class="mt-4">
						...
						<p class="capitalize"><strong>Program Studi:</strong> <?= $mhs['program_studi']; ?></p>
						<!-- <p><strong>Fakultas:</strong> Teknologi Informasi</p> -->
					</div>
					<!-- end data mahasiswa -->
							...
							<table class="items-center bg-transparent w-full border-collapse">
								<thead>
									<tr>
										...
										<th class="px-6 border border-solid border-gray-400 py-3 text-sm uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-center">Prestasi / Kegiatan</th>
										...
										foreach ($spm as $row) {
											if ($row['nim'] == $this->session->userdata('id_user')) {
												if ($kategori != $row['nama_kategori']) {
													...
													echo "<tr><td colspan='3' class='font-bold px-6 border border-solid border-gray-400 py-3 text-sm uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left'>{$row['nama_kategori']}</td></tr>";
												}
										?>
												<tr>
						...
						<div class="overflow-x-auto">
							<table class="items-center bg-transparent w-full border-collapse">
								<thead>
									<tr>
										<th class="px-6 border border-solid border-gray-400 py-3 text-sm uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-center">No</th>
										<th class="px-6 border border-solid border-gray-400 py-3 text-sm uppercase border-l-0 border-r-0 whitespace-normal font-semibold text-center">Pelanggaran Alma Ata Etiquette</th>
										<th class="px-6 border border-solid border-gray-400 py-3 text-sm uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-center">Skor</th>
									</tr>
								</thead>
								<tbody>
									...
									?>
										...
										?>
												<tr>
													<th class="border-t-0 px-6 border-l-0 border-r-0 text-sm whitespace-nowrap p-4"><?= $no; ?></th>
													<td class="border-t-0 px-6 border-l-0 border-r-0 text-sm whitespace-nowrap p-4"><?= $row['pelanggaran'] ?></td>
													<td class="border-t-0 px-6 text-center border-l-0 border-r-0 text-sm whitespace-nowrap p-4"><?= $row['poin'] ?></td>
												</tr>
									<?php
												$no++;
								...
								<tfoot>
									<tr class="bg-gray-200/50">
										<th colspan="2" class="px-6 border border-solid border-gray-400 py-3 text-sm uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-center">Total Skor</th>
										<th class="px-6 border border-solid border-gray-400 py-3 text-sm border-l-0 border-r-0 whitespace-nowrap font-bold text-center">
											<?= (!empty($etiketPoin['total_poin'])) ? $etiketPoin['total_poin'] : 'N/A' ?>
										</th>
									</tr>
								</tfoot>
							</table>
