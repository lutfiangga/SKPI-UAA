<section class="relative flex flex-col justify-between">
	...
	<section class="relative bg-[#fafafa] rounded-2xl lg:p-8 p-4 my-4">
		<form method="post" action="<?= site_url(ucwords($role) . '/Etiquette_Mahasiswa/update/' . $edit['id_etiquette']) ?>"
				...
				<div class="mb-4">
					...
					<select id="select_mahasiswa" name="nim" class="block bg-off-white w-full mt-1 p-2 border border-gray-300 rounded-md" data-search="true">
						...
						<?php foreach ($mhs as $r) { ?>
							<option value="<?= $r['nim'] ?>" <?= ($r['nim'] == $edit['nim']) ? 'selected' : '' ?>><?= $r['nim'] . ' | ' . $r['nama']; ?></option>
						<?php } ?>
					</select>
				</div>
				<div class="mb-4">
					...
					<select id="jenis_pelanggaran" required name="jenis_pelanggaran" class="block bg-off-white w-full mt-1 p-2 border border-gray-300 rounded-md" data-search="true">
						...
						<option value="sangat berat" <?= ($edit['jenis_pelanggaran'] == 'sangat berat') ? 'selected' : '' ?>>Sangat Berat</option>
					</select>
				</div>
			</div>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div class="mb-4">
					...
					<textarea id="pelanggaran" name="pelanggaran" required
						...
						placeholder="Masukkan deskripsi pelanggaran"><?= $edit['pelanggaran']; ?></textarea>
				</div>

				<div class="mb-4">
					<label for="poin" class="block text-sm font-medium text-gray-700 mb-2">Poin:</label>
					<input type="number" id="poin" name="poin" required value="<?= $edit['poin']; ?>"
			...
			<div class="mb-4">
				...
				<div class="relative file-upload-container grid grid-cols-1 md:grid-cols-2 gap-4">
					<div class="bg-white drop-zone relative cursor-pointer transition-all text-center p-8 border-blue-400 border-2 border-dashed rounded-lg  w-full shadow-lg transtition">
						...
						<input id="file" value="<?= $edit['bukti']; ?>" required name="bukti" accept="image/jpg, image/png, image/jpeg, image/wepb" type="file" class="file-input hidden" />
					...
					<div class="file-preview w-full">
						<div class="flex justify-between items-center p-2 mt-2 rounded-xl bg-[#eef0f6]">
							<div class="file-name text-blue-600 text-xs sm:text-sm md:text-base truncate max-w-xs p-2"><?= isset($edit['bukti']) ? $edit['bukti'] : ''; ?></div>
						...
						<div class="preview-container w-full flex justify-center mx-auto">
							<?php
							$file_path = FCPATH . 'assets/static/etiquette/' . $edit['bukti'];
							if (isset($edit['bukti']) && !empty($edit['bukti']) && file_exists($file_path)): ?>
								<iframe class="object-cover"
									src="<?= base_url('assets/static/etiquette/' . $edit['bukti']); ?>"
